...
def _b2i(byte): #Conversion de bytes a integrales
...
def _process_scan(raw):
    ...
    #Operacion logica del 1er valor de datos crudos and b'1'
    #Conversion a boleano
    new_scan = bool(_b2i(raw[0]) & 0b1)
    #Operación and entre b'1' y raw[0] shiftter 1 vez
    #pasado a valor entero
    inversed_new_scan = bool((_b2i(raw[0]) >> 1) & 0b1)
    #Obtencion de  la calidad: datos crudos binarito
    #Shitfter 2 hacia abajo
    quality = _b2i(raw[0]) >> 2
    if new_scan == inversed_new_scan:
        #Si el nuevo scan es igua al scan inverso
        raise RPLidarException('New scan flags mismatch')
    #Chequeo e bits raw[1] and b'1'
    check_bit = _b2i(raw[1]) & 0b1
    #Si el pareamietno de bits es diferente de 1
    if check_bit != 1:
        #Chequeo no es igual a 1
        raise RPLidarException('Check bit not equal to 1')
    #Conversion de angulo
    angle = ((_b2i(raw[1]) >> 1) + (_b2i(raw[2]) << 7)) / 64.
    #Conversion a disntancia
    distance = (_b2i(raw[3]) + (_b2i(raw[4]) << 8)) / 4.
    #Regresar scan:
    # new_scan, quality, angle, distance = new_scan, quality, angle, distance 
...
class RPLidar(object):
    ...
    def _read_descriptor(self):
        ...
        descriptor = self._serial_port.read(DESCRIPTOR_LEN) #Largo del paquete 7
        print(f"Largo: {type(descriptor)}")
        self.logger.debug('Recieved descriptor: %s', descriptor)
        if len(descriptor) != DESCRIPTOR_LEN:   #Si el largo del paquete es diferente
            raise RPLidarException('Descriptor length mismatch') 
        elif not descriptor.startswith(SYNC_BYTE + SYNC_BYTE2): #Bits iniciales incorrectos del descriptor
    ...
    def iter_measurments(self, max_buf_meas=500):
        ...
        if status == _HEALTH_STATUSES[2]:
            ...
            self.logger.warning('Warning sensor status detected! '
                                'Error code: %d', error_code)
        cmd = SCAN_BYTE     
        self._send_cmd(cmd)     #Manda comando leer un dato
        dsize, is_single, dtype = self._read_descriptor() #Descripcion del  paquete
        if dsize != 5: 
            raise RPLidarException('Wrong get_info reply length')
        if is_single:
        ...
        if dtype != SCAN_TYPE:
            raise RPLidarException('Wrong response data type')
        while True:
            #Si todo está chido, se sacan los datos crudos de  la respuesta
            raw = self._read_response(dsize) 
            #Recepcion de la respuesta tal cruda "raw"
            self.logger.debug('Recieved scan response: %s' % raw)
            if max_buf_meas: #Si un maximo de muestras existe
                data_in_buf = self._serial_port.in_waiting
                #si el databuff es mayor que el max_buf_means
                # Osea si ya están los mas de los datos para lectura 
                if data_in_buf > max_buf_meas*dsize:
                    #Mandar aviso de limpieza del buffer de entrada
                    self.logger.warning(
                        'Too many measurments in the input buffer: %d/%d. '
                        'Clearing buffer...',
                        data_in_buf//dsize, max_buf_meas)
                    #Se da lectura del buffer  tantas veces como paquetes de
                    #datos haya, para eso es data_in_buf//dsize*dsize, para obtener
                    #el numero de paquetes
                    self._serial_port.read(data_in_buf//dsize*dsize)
            #Se pasan los arrays en crudo por el procesador de datos
            #Para dejarlos bonito, de la forma:
                #new_scan, quality, angle, distance = new_scan, quality, angle, distance
    ...
    def iter_scans(self, max_buf_meas=500, min_len=5):
        ...
        '''
        #Creacion de lista scan vacio
        scan = []
        #Creacion de object generator
        iterator = self.iter_measurments(max_buf_meas)
        #por cada new_scan, quality, angle, distance en el objeto iterador
        for new_scan, quality, angle, distance in iterator:
        # Si el new_scan es VERDADERO, este sirve para declarar el nuevo
        # escan desde el primer valor.
            if new_scan:
                #si el tamaño del scan es mayor que 5
                if len(scan) > min_len:
                #Regresar escan, para concatenar uno nuevo
                    yield scan
                #Scan vacio
                scan = []
            #Si calidad es mayor a 0 y distancia es mayor a 0
            if quality > 0 and distance > 0:
                #Se van concatenando los siguientes scans
