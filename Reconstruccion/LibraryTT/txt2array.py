...
def RObjetos(DD,Epsilon,MinPts):
    ...
    """
    Separacion de clsuters por medio de DBSCAN, aqui estan las funciones necesarias
    para el programa y segmentación.
    Al finalizar la partición del clusters en posibles objetos se guardan
    los clusters resusltantes en .txt con nombre ObjetoX
    @inputs - DD son los datos del .txt
    Epsilon la distancia minima requerida por DBSCAN
    MinPts - minimo de puntos necesarios para concretar un cluster
    
    @Outputs chch que cantidad de puntos por cada cluster creado
    Primer dato es referdo a los datos outliers - si no anda bien el segmentado 
    este primer dato cuenta con la mayoria de los datos.
...
def usar(chch):
    """
    Convierte los datos cada .txt de los objetos obtenidos por DBSCAN 
    en una matriz con todos los datos de cada .txt creado
    
    @ inputs - chch cantidad de datos por cada cluster creado por DBSCAN
    
    @ outputs - TN matrix con todos los datos de cada cluster 
...
def usar2(chch,nobj):
    ...
    nobj - indica que cumulo o que objeto es (los obtenidos despues del segmentado)
    
    @output - TN es igual una matriz con cada plano, solo que esta matriz
    es solo de un cluster, no contiene los planos de cada cluster antes creado.
    
    """    
    # tplns = len(nobj)
    # # para cada objeto sacar sus planos
    # mt = int(np.amax(chch))
    # mv = len(chch)
    # TN = np.zeros((mv,mt))
    # for tt in range(0,tplns): 
        
    #     # cada plano de cada objeto
...
def imprimirObjetos(DD,TN,chch,nnn,cho):
    """
    La graficación para darme tinta de que ocurrio
    
    @inputs 
    DD - Datos del .txt, solo es usada para marcar los limites de todo el espacio 
    mapeado
    
    TN - MAtriz con clusters (obtenidos por DBSCAN o Kmeans)
    chch - vector con cantidad de datos por cada clsuter segmentado
    nnn - usado para saber como se quiere la graficación
        0 - #datos sin matriz ruido
        1 - # datos con matriz ruido
        2 - cuando solo se queire ver un clsuter en especifico
    cho - indica el cluster que se quiere visualizar se nnn es 2.
...
def fragmentos(data,nobj,nplano):
    """  
    Generacion de un archivo de texto *.txt de los planos extraidos de un cluster
    que se guardará en la carpeta "./Sets"
    
    @input          data - los datos a guardar
                    nobjt - numero de cluster al que pertenenen los planos
                    nplano - numero de plano
                    
    @output         no hay return, pero crea los archivos .txt
...
def rnsc(DD,TN,chch,abcd,ldps,gplns):
    """
    Función para usar el RANSAC, el metodo de ajuste a una ecuación, 
    en este caso a un planp a partir de obtener la normal al plano por 3 puntos aleatorios
    
    @inputs-  DD Datos totales per aqui es usado para funcion graficacion
              TN Matriz con clusters
              chch vector con cantidad de datos por cada cluster
              abcd para guardar los valores (a,b,c,d) de cada plano
              ldps vector para guardar cantidad de datos por cada plano
              gplns vector para guardar cantidad de planos por cada cluster
    
    @Outputs - abcd,ldps,gplns con todos los datos de todos los planos
    ...
    # while Plns<=1:
        ...
        imprimirObjetos(DD,Ps,ldplano,1,0)
...
def bytxt():
    ...
    """
    Función que eliminar los .txt de cada objeto y cada plano en
    carpeta /Sets, esto para que cada nueva vuelta del proceso los datos 
    pasados no esten arruinando 
...
def rnsc2(DD,TN,chch,abcd,ldps,gplns):   
    """
    Función para usar el RANSAC, el metodo de ajuste a una ecuación, 
    en este caso a un planp a partir de obtener la normal al plano por 3 puntos aleatorios
    
    @inputs-  DD Datos totales per aqui es usado para funcion graficacion
              TN Matriz con el cluster a usar
              chch cantidad de datos del cluster
              abcd para guardar los valores (a,b,c,d) de cada plano
              ldps vector para guardar cantidad de datos por cada plano
              gplns vector para guardar cantidad de planos del cluster
    ...
    Cl = np.copy(TN[0:int(chch),:]) # Creamos copia de datos para no afectar a los originales
        # Cl = np.copy(TN[objts,0:int(chch[objts]),:])
...
def kk(Cl,k):
    """
    Función del algoritmo de segmentación kmeans
    Cada cluster obtenido se guarda en un .txt
    
    @inputs  Cl - Cluster a segmentar
             k - Cantidad de clusters que se requieren (valor prueba y error)
    
    @ output
            TN - Matriz con todos los datos de todos los clusters
            chch - cantidad de datos para cada cluster.
